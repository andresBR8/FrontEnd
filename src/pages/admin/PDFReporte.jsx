...
const CustodyDocument = ({ title, data, activos }) => {
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <Text style={styles.fechaHora}>Fecha: {fechaActual}</Text>
        <Text style={styles.title}>{title}</Text>
        <View style={styles.details}>
          <Text>Nombre: {data.nombre}</Text>
          <Text>Cargo: {data.cargo}</Text>
          <Text>Unidad: {data.unidad}</Text>
          <Text>Descripción del Reporte: {data.descripcionReporte}</Text>
        </View>
        <View style={styles.table}>
          <View style={styles.tableRow}>
            {/* Encabezados de la tabla */}
            <View style={{ ...styles.tableColHeader, width: '10%' }}><Text style={styles.tableCell}>ID</Text></View>
            <View style={{ ...styles.tableColHeader, width: '15%' }}><Text style={styles.tableCell}>Código</Text></View>
            <View style={{ ...styles.tableColHeader, width: '20%' }}><Text style={styles.tableCell}>Nombre del Modelo</Text></View>
            <View style={{ ...styles.tableColHeader, width: '20%' }}><Text style={styles.tableCell}>Estado</Text></View>
            <View style={{ ...styles.tableColHeader, width: '15%' }}><Text style={styles.tableCell}>Costo</Text></View>
          </View>
          {activos.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <View style={{ ...styles.tableCol, width: '10%' }}><Text style={styles.tableCell}>{item.id}</Text></View>
              <View style={{ ...styles.tableCol, width: '15%' }}><Text style={styles.tableCell}>{item.codigo}</Text></View>
              <View style={{ ...styles.tableCol, width: '20%' }}><Text style={styles.tableCellLeft}>{item.activoModelo.nombre}</Text></View>
              <View style={{ ...styles.tableCol, width: '20%' }}><Text style={styles.tableCell}>{item.activoModelo.estado}</Text></View>
              <View style={{ ...styles.tableCol, width: '15%' }}><Text style={styles.tableCell}>{item.activoModelo.costo}</Text></View>
            </View>
          ))}
        </View>
      </Page>
